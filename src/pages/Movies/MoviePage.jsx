...
const MoviePage = () => {
  ...
  const [sort, setSort] = useState(""); // 셀렉트박스에 보여질 선택된 정렬값 저장
  const keyword = query.get("q");
  const [page, setPage] = useState(1); // 페이지네이션에 보여질 선택된 페이지넘버 저장
  const [data, setData] = useState(null);
  const [genre, setGenre] = useState(null); // 셀렉트박스에 보여질 선택된 장르값 저장
